# -*- coding: utf-8 -*-
# @Author  : Lee
# @Time    : 2021/8/11 21:42
# @Function: 3.5.2. 学习曲线
# 学习曲线显示了对于不同数量的训练样本的估计器的验证和训练评分。
# 它可以帮助我们发现从增加更多的训 练数据中能获益多少，以及估计是否受到更多来自方差误差或偏差误差的影响。
#
# 在第一列中，第一行显示了数字数据集的朴素贝叶斯分类器的学习曲线。请注意，培训分数和交叉验证分数在最后都不是很好。
# 如果在增加训练集大小时，验证分数和训练分数都收敛到一个很低的值，那么我们将不会从更多的训练数据中获益。
# 然而，曲线的形状通常可以在更复杂的数据集中找到：训练分数在开始时非常高，然后下降；交叉验证分数在开始时非常低，然后上升。

# 在第二列中，第一行我们看到了带有RBF核的SVM的学习曲线。
# 我们可以清楚地看到，训练分数仍然在最大值附近，并且验证分数可以随着训练样本的增加而增加。

# 第二行中的图显示了模型使用各种大小的训练数据集进行训练所需的时间。
# 第三行中的曲线图显示了为每个训练规模训练模型所需的时间。
