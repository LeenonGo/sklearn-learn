...
# @Author  : Lee
# @Time    : 2021/8/7 23:17
# @Function: 4. 鲁棒协方差估计  https://www.scikitlearn.com.cn/0.21.3/25/#264
# 
# 实际数据集通常是会有测量或记录错误。常规但不常见的观察也可能出于各种原因。
# 每个不常见的观察称为异常值(outliers)。上面提出的经验协方差估计器和收缩协方差估计器对数据中异常值非常敏感。
# 因此，应该使用鲁棒协方差估计（robust covariance estimators）来估算其真实数据集的协方差。
# 或者，可以使用鲁棒协方差估计器来执行异常值检测， 并根据数据的进一步处理:丢弃/降低某些观察值。
