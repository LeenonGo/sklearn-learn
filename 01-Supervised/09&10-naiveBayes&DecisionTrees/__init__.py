...
# 朴素贝叶斯：https://www.scikitlearn.com.cn/0.21.3/10/
# 特别是文档分类和垃圾邮件过滤。
# 相比于其他更复杂的方法，朴素贝叶斯学习器和分类器非常快。


# 决策树： https://www.scikitlearn.com.cn/0.21.3/11/
...
#   能够处理多路输出的问题。
#   使用白盒模型。
#       如果某种给定的情况在该模型中是可以观察的，那么就可以轻易的通过布尔逻辑来解释这种情况。
#       相比之下，在黑盒模型中的结果就是很难说明清楚地。
#   可以通过数值统计测试来验证该模型。这对事解释验证该模型的可靠性成为可能。
#   即使该模型假设的结果与真实模型所提供的数据有些违反，其表现依旧良好。

# 决策树的缺点包括:
#   决策树模型容易产生一个过于复杂的模型,这样的模型对数据的泛化性能会很差。
#       这就是所谓的过拟合.一些策略像剪枝、设置叶节点所需的最小样本数或设置数的最大深度是避免出现 该问题最为有效地方法。
#   决策树可能是不稳定的，因为数据中的微小变化可能会导致完全不同的树生成。这个问题可以通过决策树的集成来得到缓解
#   在多方面性能最优和简单化概念的要求下，学习一棵最优决策树通常是一个NP难问题。
#       因此，实际的决策树学习算法是基于启发式算法，例如在每个节点进 行局部最优决策的贪心算法。
#       这样的算法不能保证返回全局最优决策树。
...
# 决策树在高维空间中只有少量的样本的树是十分容易过拟合，事先进行降维
